...
add_action('wp_enqueue_scripts', 'dss_readspeaker_cssjs');
function dss_readspeaker_cssjs() {
	...
	$srcproto = 'http';
	$srcparams = 'pids=embhl';
	if ($dss_readspeaker_settings['rshttps']) $srcproto = 'https';
	if ($dss_readspeaker_settings['rscompact']) $srcparams = 'pids=embhl&amp;skin=ReadSpeakerCompactSkin';
	if ($dss_readspeaker_settings['rshttps']) {
		wp_enqueue_script('readspeaker', plugins_url( 'ReadSpeaker/ReadSpeaker.js?pids=embhl', __FILE__ ), 'jquery' );
	}
	else{
		wp_enqueue_script('readspeaker', $srcproto. '://f1.'.$region.'.readspeaker.com/script/'.$customerid.'/ReadSpeaker.js?'.$srcparams, 'jquery', NULL );
	}
	
	// Custom styles
	if ($dss_readspeaker_settings['rsstyles']) {
		wp_enqueue_script('dss_readspeaker_js', plugins_url('readspeaker.js', __FILE__));
		wp_enqueue_style('dss_readspeaker_css', plugins_url('readspeaker.css', __FILE__));
	}
}
...
function dss_readspeaker_render() { 
	...
	if (
		...
		<?php }
		elseif  ($dss_readspeaker_settings['rshttps']){ ?>
			<div id="readspeaker_button" class="rs_skip rsbtn_colorskin rs_preserve">
    			<a class="rsbtn_play" href="<?php echo $srcproto ?>://app.readspeaker.com/cgi-bin/rsent?customerid=<?php echo $dss_readspeaker_settings['customerid']; ?>&amp;lang=<?php echo $dss_readspeaker_settings['rslang']; ?>&amp;readid=<?php echo $dss_readspeaker_settings['readid']; ?>&amp;url=<?php echo $srcproto ?>://<?php echo $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"] ?>">
			<span class="rsbtn_left rspart rsimg"><span class="rsbtn_text"><span>Listen</span></span></span><span class="rsgrad"><span class="rsbtn_right rsplay rspart"></span></span>
			</a>
			</div>
		<?php }
...
function dss_readspeaker_page() {
	...
	if(isset($_POST['action'])) {		
		...
		if ($_POST['action'] == 'update') {
			$dss_readspeaker_settings = array(
				...
				'region' => isset($_POST['region']) ? $_POST['region'] : 'as',
				...
				'rshttps' => isset($_POST['rshttps']) ? true : false,
				'rscompact' => isset($_POST['rscompact']) ? true : false,
			);
	...
	<fieldset><legend>Advanced</legend>	
		<p><input id="rshttps" type="checkbox" name="rshttps" <?php if ($dss_readspeaker_settings['rshttps']) echo ' checked="checked"'; ?> />
    <label for="rshttps">Use https://</label><small>Connects to Readspeaker's CDN with the https protocol</small></p>	
	...
	<p><input id="rscompact" type="checkbox" name="rscompact" <?php if ($dss_readspeaker_settings['rscompact']) echo ' checked="checked"'; ?> />
    <label for="rscompact">Use compact player</label><small>Reduced width, if space is at a premium</small></p>	
	</fieldset>
